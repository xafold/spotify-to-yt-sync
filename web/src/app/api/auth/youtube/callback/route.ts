...
export async function GET(request: NextRequest) {
  ...
  if (error || !code) {
    ...
    return NextResponse.redirect(
      `${origin}/setup?error=youtube_denied&reason=${encodeURIComponent(reason)}`
    );
  }
  ...
  if (!tokens?.access_token || !tokens?.refresh_token) {
    return NextResponse.redirect(
      `${origin}/setup?error=youtube_token_exchange_failed`
    );
  }
  ...
  if (!channel) {
    return NextResponse.redirect(
      `${origin}/setup?error=youtube_channel_failed`
    );
  }
  ...
  return NextResponse.redirect(`${origin}/setup?youtube=connected`);
}
